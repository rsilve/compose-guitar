...
suite("track callback", () => {
    ...
    test("new track", async () => {
        const state = await track_callback(new Action(TRACK_NEW), {...st, transpose: 1});
    ...
    test("new track without save", async () => {
        const state = await track_callback(new Action(TRACK_NEW_WITHOUT_SAVE), {...st, transpose: 1});
        expect(state.editor).not.to.be.undefined
        expect(state.confirm_save).to.be.undefined
        expect(state.transpose).to.be.equal(0)
    })
    ...
    test("new track cancel", async () => {
        const state = await track_callback(new Action(TRACK_NEW_CANCEL), {...st, transpose: 1});
        expect(state.editor).to.be.undefined
        expect(state.confirm_save).to.be.undefined
    ...
    test("new track cancel (modal close", async () => {
        const state = await track_callback(new Action(MODALS_CLOSE), {...st, transpose: 1});
        expect(state.editor).to.be.undefined
        expect(state.confirm_save).to.be.undefined
        expect(state.transpose).to.be.equal(1)
    })
    ...
    test("edit track", async () => {
        ...
        const state = await track_callback(new Action(TRACK_EDIT, payload), {...st});
        expect(state.editor).not.to.be.undefined
    ...
    test("cancel track edition", async () => {
        const {track = {}} = st
        const state = await track_callback(new Action(TRACK_EDIT_CANCEL), {...st});
        expect(state.editor).to.be.undefined
