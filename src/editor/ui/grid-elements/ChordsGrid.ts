...
class ChordsGrid extends LitElement {
  static styles = [styles,
    ...
    render(): unknown {
      ...
      const map = grid.rows.map((row) => html`
                <tr style="border-collapse: collapse">${this.renderRow(row)}</tr>`);
      return html`
            <table class="grid">
                <tbody>${map}</tbody>
            </table>`;
    ...
    renderRow(row: Row): unknown {
      const map = row.measure.map((measure) => html`
                <td class="grid grid-measure">${this.renderMeasure(measure)}</td>`);
      return html`${map}`;
    }
