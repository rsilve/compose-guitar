...
suite("Measure1", () => {
    ...
    test('is defined', async () => {
        const el = await fixture(html`
            <chords-grid-measure1></chords-grid-measure1>`)
    ...
    test('is defined with measure', async () => {
        const el = await fixture(html`
            <chords-grid-measure1 .measure="${new Measure("A")}"></chords-grid-measure1>`)
        await expect(el).shadowDom.to.be.accessible();
        expect(el).shadowDom.to.be.equal(`
    ...
    test('is defined with measure and transpose', async () => {
        const el = await fixture(html`
            <chords-grid-measure1 .measure="${new Measure("A")}" transpose="2"></chords-grid-measure1>`)
        await expect(el).shadowDom.to.be.accessible();
        expect(el).shadowDom.to.be.equal(`
        <table class="grid-measure-1">
                <tbody>
